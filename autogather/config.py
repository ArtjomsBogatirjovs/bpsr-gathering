...
APPROACH_STEP_MS   = 180    # сколько держать клавишу при подбегании (мс)
APPROACH_PAUSE     = 0.08   # пауза между шагами
APPROACH_TOLERANCE = 40     # пикселей — считаем "дошли", если ресурс почти в центре
# Когда на кадре нет руды — небольшое "подталкивание" камеры по X
NUDGE_MOUSE_X_PX   = 140

# ===== Калибровка бега по расстоянию в пикселях =====
# Сколько миллисекунд удерживать клавишу на 1 пиксель смещения цели
MS_PER_PX_STRAFE = 1.10   # A/D — подгон по X (влево/вправо)
MS_PER_PX_FORWARD = 0.95  # W/S — подгон по Y (вперёд/назад)

# Ограничители, чтобы не улетать слишком далеко одним рывком
APPROACH_MIN_MS = 80       # минимум удержания
APPROACH_MAX_MS = 900      # максимум удержания за один вызов по каждой оси
...
NODE_REACH_TOLERANCE_PX= 45     # считаем, что «дошли до узла», если ближе этого
NODE_MAX_CHASE_PX      = 1800   # (опц.) не гнаться слишком далеко, 0 = без лимита
